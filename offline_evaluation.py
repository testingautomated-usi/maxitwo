...
if __name__ == "__main__":
    ...
    assert (
        args.model_name in args.model_path
    ...
    filename = (
        "offline-evaluation-fake-{}-{}.npz".format(args.env_name, args.model_name)
        if "fake" in args.model_path
        else "offline-evaluation-{}-{}.npz".format(args.env_name, args.model_name)
    )
    ...
    if "tracks_control_points" in numpy_dict:
        ...
        logger.debug(
            "Tracks control points shape: {}".format(
                new_numpy_dict["tracks_control_points"].shape
            )
        )
    else:
    ...
    if num_episodes > 0:
        ...
        logger.debug(
            f"Average: {running_mean / len(new_numpy_dict['prediction_errors'])} "
            f"+- {running_std / len(new_numpy_dict['prediction_errors'])}"
        )
    else:
        logger.debug(
            f"Average: {np.mean(new_numpy_dict['prediction_errors'])} "
