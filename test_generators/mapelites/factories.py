...
def make_features_given_concrete_features(
    concrete_features: List[Tuple[str, Union[int, float]]]
) -> List[Feature]:
    features = []
    for concrete_feature in concrete_features:
        feature_name, feature_value = concrete_feature[0], concrete_feature[1]
        if (
            feature_name == STD_STEERING_ANGLE_FEATURE_NAME
            or feature_name == OLD_STD_STEERING_ANGLE_FEATURE_NAME
        ):
            features.append(
                StdSteeringAngleFeature(
                    steering_angles=[], steering_angle_bin=feature_value
                )
            )
        elif feature_name == CURVATURE_FEATURE_NAME:
            features.append(
                CurvatureFeature(curvature=None, curvature_bin=feature_value)
            )
        elif feature_name == TURNS_COUNT_FEATURE_NAME:
            features.append(
                TurnsCountFeature(turns_count=None, turns_count_bin=feature_value)
            )
        else:
            raise NotImplementedError("Feature {} not supported".format(feature_name))
    return features
...
def make_features_given_feature_combination(
    ...
    if feature_combination == "{}-{}".format(
        STD_STEERING_ANGLE_FEATURE_NAME, CURVATURE_FEATURE_NAME
    ):
        return (
            StdSteeringAngleFeature(steering_angles=steering_angles, mock=mock),
            (
                CurvatureFeature(
                    curvature=individual.get_representation().compute_curvature()
                )
                if not mock
    ...
    if feature_combination == "{}-{}".format(
        ...
        return (
            (
                TurnsCountFeature(turns_count=num_turns)
                if not mock
                else TurnsCountFeature(turns_count=None, mock=mock)
            ),
            (
                CurvatureFeature(
                    curvature=individual.get_representation().compute_curvature()
...
def make_quality_metric(
    quality_metric: str, behavioural_metric_name: str, behavioural_metrics: Dict
) -> Union[float, int]:
