...
parser.add_argument(
    "--archive-path",
    help="Path of the folder where the logs are",
    type=str,
    default="logs",
)
parser.add_argument(
...
parser.add_argument(
    "--show-plot", help="Show histograms", action="store_true", default=False
)
...
def get_bin_positions(bins_edges: List[float]) -> List[float]:
    ...
    for i in range(len(bins_edges) - 1):
        if len(bins_positions) == 0:
            ...
            bins_positions.append(
                bins_positions[-1] + bins_edges[i + 1] - bins_edges[i]
            )
...
if __name__ == "__main__":
    ...
    assert (
        len(args.archive_names) <= 3
    ), "Cannot compute offline metrics among more than 3 maps"
    ...
    for i, archive_name in enumerate(args.archive_names):
        numpy_dict = load_archive(
            archive_path=args.archive_path, archive_name=archive_name
        )
        ...
        if type(ith_predictions_errors[0]) == np.ndarray:
            for prediction_errors in ith_predictions_errors:
                if args.predict_throttle:
                    ...
                    concatenated_ith_prediction_errors.extend(
                        [prediction_error for prediction_error in prediction_errors]
                    )
        elif (
            type(ith_predictions_errors[0]) == np.float64
            or type(ith_predictions_errors[0]) == np.float32
        ):
            concatenated_ith_prediction_errors.extend(ith_predictions_errors)
        else:
            raise RuntimeError(
                "Unknown type of prediction error: {}".format(
                    type(ith_predictions_errors[0])
                )
            )
    ...
    if len(args.archive_names) == 3:
        ...
        logg.info(
            "Cohen's d effect size among prediction errors: {}, {}".format(
                estimate, magnitude
            )
        )
        ...
        if p_value > 0.05 and abs(estimate) > 0.0:
            power = analysis.power(
                ...
                nobs1=len(prediction_errors_merged) + len(prediction_errors_3),
                alpha=alpha,
            )
            logg.info("Parametric power at alpha {}: {}".format(alpha, power))

            nobs = analysis.solve_power(
                effect_size=estimate, power=desired_power, alpha=alpha
            )
            logg.info(
                ...
                "Current number of observations: {}".format(
                    ...
                    alpha,
                    nobs,
                    len(prediction_errors_merged) + len(prediction_errors_3),
                )
            )
        ...
        if p_value > 0.05 and abs(estimate) > 0.0:
            ...
            power = analysis.power(
                effect_size=estimate,
                nobs1=len(prediction_errors_1) + len(prediction_errors_2),
                alpha=alpha,
            )
            logg.info("Parametric power at alpha {}: {}".format(alpha, power))
            ...
            logg.info(
                ...
                "Current number of observations: {}".format(
                    ...
                    alpha,
                    nobs,
                    len(prediction_errors_1) + len(prediction_errors_2),
                )
            )
