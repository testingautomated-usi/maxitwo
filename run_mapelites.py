...
from config import SIMULATOR_NAMES, AGENT_TYPES, NUM_CONTROL_NODES, MAX_ANGLE, NUM_SAMPLED_POINTS, MIN_ANGLE, \
    MOCK_SIM_NAME, BEAMNG_SIM_NAME
from envs.beamng.config import MAP_SIZE
from factories import make_env, make_agent, make_test_generator
from global_log import GlobalLog
from test_generators.mapelites.config import TURNS_COUNT_FEATURE_NAME, CURVATURE_FEATURE_NAME, FEATURE_COMBINATIONS
...
parser.add_argument('--min-angle', help="Min angle of a curve of the generated road (only valid with random generator)", type=int, default=MIN_ANGLE)
...
parser.add_argument('--cyclegan', help="Whether to load the cyclegan", action="store_true", default=False)
# mapelites parameters
parser.add_argument('--population-size', help="Size of the initial population", type=int, default=40)
...
parser.add_argument('--iteration-runtime', help="Search budget of the algorithm (in number of iterations)", type=int, default=60)
parser.add_argument('--mutation-extent', help="How much each control point coordinate can be changed", type=int, default=6)
parser.add_argument('--num-runs', help="Number of times the search should be executed. If > 0 then it also computes the probability map.", type=int, default=-1)
parser.add_argument('--resume-datestr', help="Date string needed to resume previous execution.", type=str, default=None)
...
if __name__ == '__main__':
    ...
    if args.num_runs == -1:
        ...
        range_num_runs = range(args.num_runs) if args.resume_run == -1 else range(args.resume_run, args.num_runs)
