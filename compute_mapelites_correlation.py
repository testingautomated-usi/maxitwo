...
from utils.report_utils import load_mapelites_report, resize_map_of_elites, get_name_min_and_max_2d_features, \
    load_raw_map, resize_map, plot_roc_curve
...
parser.add_argument('--multiply-probabilities', help='Multiply probabilities when merging the probability maps (by default the probabilities are averaged)', action="store_true", default=False)
parser.add_argument('--weighted-average-probabilities', help='Whether to consider a weighted average of the probabilities', action="store_true", default=False)
...
parser.add_argument('--only-failures', help='Whether to consider only failures when computing the correlation between two maps', action="store_true", default=False)
parser.add_argument('--quality-metric', help='Name of the quality metric', type=str, choices=QUALITY_METRICS_NAMES, default=None)
parser.add_argument('--quality-metric-merge', help='How to merge the quality metric maps', type=str, choices=["avg", "min", "max"], default=None)
parser.add_argument('--failure-predicts-quality-metric', help='Whether failure probability predicts quality metric. If "no" then quality metric predicts failure probability', type=str, choices=["yes", "no"], default="yes")
...
if __name__ == "__main__":
    ...
    for filepath in args.filepaths:
        assert os.path.exists(os.path.join(args.folder, filepath)), \
    ...
    if args.load_probability_map or args.quality_metric is not None:
        ...
        for i, report in enumerate(reports):
            ...
            logg.info("Report #{}, all individuals length: {}, individuals in population: {}".format(
                i, len(report["individuals"]), len(report["ids_in_population"]))
            )

        feature_x_name, feature_y_name, min_feature_x, max_feature_x, min_feature_y, max_feature_y = \
            get_name_min_and_max_2d_features(individuals=all_individuals_in_population)

        logg.info("Feature x: {} in [{}, {}], Feature y: {} in [{}, {}]".format(
    ...
    assert len(elements_equal) == 0, \
        'Find another value rather than {} to replace {} for correlation computation. -2 is present in the map: {}'\
            .format(replace_fill_value_with_value, fill_value, flatten_m1)

    elements_equal = np.where(flatten_m2 == replace_fill_value_with_value)[0]
    assert len(elements_equal) == 0, \
        'Find another value rather than {} to replace {} for correlation computation. -2 is present in the map: {}' \
            .format(replace_fill_value_with_value, fill_value, flatten_m2)
    ...
    if args.load_probability_map or args.quality_metric is not None:
        ...
        assert np.alltrue(np.equal(indices_m1_with_fill_value, indices_m2_with_fill_value)), \
    ...
    for i, val in enumerate(flatten_m1):
        if val != replace_fill_value_with_value \
                and flatten_m2[i] != replace_fill_value_with_value:
            if args.only_failures:
                if (args.failure_probability and val > 0.0 and flatten_m2[i] > 0.0) \
                        or (not args.failure_probability and val < 1.0 and flatten_m2[i] < 1.0):
    ...
    corr, p_value = pearsonr(x=flatten_m1, y=flatten_m2)
    logg.info("Correlation not filtered: {}, p-value {}".format(corr, p_value))
    ...
    if args.quality_metric is not None:
        ...
        if args.weighted_average_probabilities:
            auc_roc_filename += '_weighted'
    ...
    logg.debug('Precision values: {}'.format(list(precision)))
    logg.debug('Recall values: {}'.format(list(recall)))
    logg.debug('Precision-recall baseline: {} ({}/{})'.format(
