...
if __name__ == "__main__":
    ...
    if args.load_probability_map or args.quality_metric is not None:
        ...
        for i, report in enumerate(reports):
            ...
            ser = pd.Series(
                list(resized_map.values()),
                index=pd.MultiIndex.from_tuples(resized_map.keys()),
            )
            df = ser.unstack()
    ...
    assert (
        m1.shape == m2.shape
    ), "The two maps do not have the same shape: {} != {}".format(m1.shape, m2.shape)
    ...
    flatten_m1 = np.where(
        flatten_m1 == fill_value, replace_fill_value_with_value, flatten_m1
    )
    flatten_m2 = np.where(
        flatten_m2 == fill_value, replace_fill_value_with_value, flatten_m2
    )

    if args.load_probability_map or args.quality_metric is not None:
        indices_m1_with_fill_value = np.where(
            flatten_m1 == replace_fill_value_with_value
        )[0]
        indices_m2_with_fill_value = np.where(
            flatten_m2 == replace_fill_value_with_value
        )[0]
        assert np.alltrue(
    ...
    for i, val in enumerate(flatten_m1):
        if (
            val != replace_fill_value_with_value
    ...
    logg.info(
        "Num values in map: {}. Num values in filtered map: {}".format(
            len(flatten_m1), len(filtered_m1)
        )
    )
    ...
    auc_roc_curve_filepath = os.path.join(
        args.folder, "{}_{}_{}.pdf".format(auc_roc_filename, filepath1, filepath2)
    )
    ...
    average_precision = average_precision_score(
        y_true=ground_truth, y_score=filtered_m1
    )
    logg.info("Average precision: {}".format(average_precision))
    precision, recall, thresholds = precision_recall_curve(
        y_true=ground_truth, probas_pred=filtered_m1
    )
    ...
    logg.debug(
        "Precision-recall baseline: {} ({}/{})".format(
            np.sum(ground_truth) / len(ground_truth),
            np.sum(ground_truth),
            len(ground_truth),
        )
    )
