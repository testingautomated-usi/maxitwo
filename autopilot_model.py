...
from tensorflow.keras.layers import Lambda, Conv2D, Dropout, Flatten, Dense, MaxPooling2D
...
class AutopilotModel:
    ...
    def __init__(self, env_name: str, input_shape: Tuple[int] = INPUT_SHAPE, predict_throttle: bool = True):
        ...
        assert env_name in SIMULATOR_NAMES or env_name == 'mixed', \
    ...
    def _build_chauffeur_model(self, keep_probability: float = 0.1) -> Sequential:
        ...
        model = Sequential()
        model.add(Lambda(lambda x: x / 127.5 - 1.0, input_shape=self.input_shape))
        model.add(Conv2D(16, (5, 5), input_shape=self.input_shape, kernel_initializer="he_normal",
                         bias_initializer="he_normal", activation='relu', padding='same'))
        model.add(Dropout(0.05))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(20, (5, 5), kernel_initializer="he_normal", bias_initializer="he_normal", activation='relu',
                         padding='same'))
        model.add(Dropout(0.05))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(40, (3, 3), kernel_initializer="he_normal", bias_initializer="he_normal", activation='relu',
                         padding='same'))
        model.add(Dropout(0.05))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(60, (3, 3), kernel_initializer="he_normal", bias_initializer="he_normal", activation='relu',
                         padding='same'))
        model.add(Dropout(0.05))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(80, (2, 2), kernel_initializer="he_normal", bias_initializer="he_normal", activation='relu',
                         padding='same'))
        model.add(Dropout(0.05))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(128, (2, 2), kernel_initializer="he_normal", bias_initializer="he_normal", activation='relu',
                         padding='same'))
        model.add(Flatten())
        model.add(Dropout(keep_probability))
    ...
    def train_model(
        ...
        self.model = self.build_model(model_name=model_name, keep_probability=keep_probability)
        ...
        history = self.model.fit(
            ...
            max_queue_size=10,
            workers=8,
            callbacks=[checkpoint, early_stopping, learning_rate_scheduler] if decay_learning_rate else [checkpoint, early_stopping],
            verbose=1
        )
        ...
        if save_plots:
            ...
            plt.savefig(
                os.path.join(
                    ...
                    if not fake_images else f"{self.env_name}-loss-{datetime_str}-fake-{model_name}.pdf"
                )
            )
