    ...
    def visualize_road_test(
        ...
        if not invert:
            road_poly = LineString(
                [(t[0], t[1]) for t in road.get_concrete_representation(to_plot=True)]
        ...
        transformed_fov = rotate(
            self.little_triangle, origin=(0, 0), angle=rotation_angle
        )
        ...
        if plot_control_points:
            ...
            plt.scatter(
                control_points_xs, control_points_ys, color="red", marker="*", s=50
            )
