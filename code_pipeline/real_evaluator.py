...
from config import BEAMNG_SIM_NAME, DONKEY_SIM_NAME, MAX_EPISODE_STEPS
from custom_types import GymEnv
from envs.beamng.config import MAP_SIZE
from factories import make_agent, make_env
...
class RealEvaluator(Evaluator):
    def __init__(self, env_name: str, env: GymEnv, agent: Agent, feature_combination: str, collect_images: bool = False):
    ...
    def run_sim(self, individual: Individual) -> None:
        ...
        while not done:
            ...
            if episode_length > MAX_EPISODE_STEPS:
                self.logger.warn(
                    ...
                    "a communication error happenend".format(episode_length, MAX_EPISODE_STEPS)
                )
                raise RuntimeError()
        ...
        individual.set_fitness(LateralPositionFitness(lateral_positions=lateral_positions))

        individual.set_features(
            features=make_features_given_feature_combination(
                feature_combination=self.feature_combination, steering_angles=steering_angles, individual=individual
            )
        )
...
if __name__ == "__main__":
    ...
    agent = make_agent(env_name=env_name, env=env, model_path=model_path, agent_type="supervised", predict_throttle=False)
    ...
    for i in range(5):
        ...
        print(
            "Fitness: {}. Features: {}".format(
                individual.get_fitness().get_value(), [feature.get_value() for feature in individual.get_features()]
            )
        )
