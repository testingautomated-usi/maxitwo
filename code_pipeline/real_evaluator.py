...
from config import DONKEY_SIM_NAME, BEAMNG_SIM_NAME, MAX_EPISODE_STEPS, UDACITY_SIM_NAME, MAX_EPISODE_STEPS_UDACITY
    ...
    def run_sim(self, individual: Individual) -> None:
        ...
        while not done:
            ...
            if episode_length > MAX_EPISODE_STEPS \
                    or (self.env_name == UDACITY_SIM_NAME and episode_length > MAX_EPISODE_STEPS_UDACITY):
                self.logger.warn("Episode length {} > {}. Either the simulator went in background or "
                                 "a communication error happened".format(
                    episode_length, MAX_EPISODE_STEPS
                    if episode_length > MAX_EPISODE_STEPS else MAX_EPISODE_STEPS_UDACITY)
                )
                raise RuntimeError()
