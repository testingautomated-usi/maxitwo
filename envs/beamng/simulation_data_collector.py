...
from envs.beamng.oob_monitor import OutOfBoundsMonitor
from envs.beamng.simulation_data import (
    SimulationParams,
...
class SimulationDataCollector:
    ...
    def collect_current_data(self, oob_bb=True, wrt="right", save_images: bool = False):
        ...
        if save_images:
            img_center = np.array(
                self.vehicle_state_reader.sensors["cam_center"]["colour"]
            )
            img_left = np.array(self.vehicle_state_reader.sensors["cam_left"]["colour"])
            img_right = np.array(
                self.vehicle_state_reader.sensors["cam_right"]["colour"]
            )
            self.simulation_data.images.append((img_center, img_left, img_right))

        is_oob, oob_counter, max_oob_percentage, oob_distance, oob_percentage = (
    ...
    def take_car_picture_if_needed(self):
        ...
        if last_state.is_oob:
            ...
            img_path = self.simulation_data.path_root.joinpath(
                f"oob_camera_shot{last_state.oob_counter}.jpg"
            )
