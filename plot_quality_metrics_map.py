...
from utils.report_utils import build_quality_metrics_map_from_reports, FILL_VALUE, \
    build_quality_metrics_map_from_report, plot_raw_map_of_elites, plot_map_of_elites, get_values_from_individuals
...
parser.add_argument('--datetime-str-beamng', help='Datetime string used in the search folders of beamng', type=str, required=True)
parser.add_argument('--datetime-str-udacity', help='Datetime string used in the search folders of udacity', type=str, required=True)
parser.add_argument('--datetime-str-donkey', help='Datetime string used in the search folders of donkey', type=str, default=None)
...
if __name__ == "__main__":
    ...
    folder = os.path.join(args.folder, args.filepath)
    ...
    if args.datetime_str_donkey is not None:
        str_datetime_env_name_num_runs_list.append(
            ("{}".format(args.datetime_str_donkey), DONKEY_SIM_NAME, 5)
        )

    # FIXME: for simplicity at the moment the folders are hardcoded
    migration_folders_env_names = [
        ("{}/mapelites_migration_beamng_udacity_search".format(folder), BEAMNG_SIM_NAME),
        ("{}/mapelites_migration_udacity_beamng_search".format(folder), UDACITY_SIM_NAME),
        ("{}/mapelites_migration_donkey_beamng_search".format(folder), DONKEY_SIM_NAME),
        ("{}/mapelites_migration_donkey_udacity_search".format(folder), DONKEY_SIM_NAME),
    ]
    if args.datetime_str_donkey is not None:
        migration_folders_env_names.extend([
            ("{}/mapelites_migration_beamng_donkey_search".format(folder), BEAMNG_SIM_NAME),
            ("{}/mapelites_migration_udacity_donkey_search".format(folder), UDACITY_SIM_NAME)
        ])
    ...
    for migration_folder, env_name in migration_folders_env_names:
        ...
        for quality_metric in QUALITY_METRICS_NAMES:
            ...
            quality_metric_map, individuals, feature_name_x, feature_name_y = \
                build_quality_metrics_map_from_report(filepath=report_file, quality_metric=quality_metric)
            ...
            logg.info("Bounds {} for {}: ({}, {})".format(
                quality_metric,
                os.path.join(folder, migration_folder),
                min_quality_metric,
                max_quality_metric
            ))
            quality_metric_dict[quality_metric] = (
    ...
    for env_name, quality_metric_dicts in quality_metric_dicts_across_simulators.items():
        if args.datetime_str_donkey is not None:
            # cross-validation
            assert len(quality_metric_dicts) == 3, \
                "{} has more or less than 3 folders. Found: {}".format(env_name, len(quality_metric_dicts))
        else:
            assert len(quality_metric_dicts) == 2, \
                "{} has more or less than 2 folders. Found: {}".format(env_name, len(quality_metric_dicts))
            ...
            for quality_metric in quality_metric_dict.keys():
                ...
                for key in quality_metric_map.keys():
                    if quality_metric_map[key] != FILL_VALUE:
                        ...
                        assert 0 <= quality_metric_map[key] <= 1, \
