...
parser.add_argument(
    "--datetime-str-beamng",
    help="Datetime string used in the search folders of beamng",
    type=str,
    required=True,
)
parser.add_argument(
    "--datetime-str-udacity",
    help="Datetime string used in the search folders of udacity",
    type=str,
    required=True,
)
parser.add_argument(
    "--datetime-str-donkey",
    help="Datetime string used in the search folders of donkey",
    type=str,
    default=None,
)
...
if __name__ == "__main__":
    ...
    migration_folders_env_names = [
        (
            "{}/mapelites_migration_beamng_udacity_search".format(folder),
            BEAMNG_SIM_NAME,
        ),
        (
            "{}/mapelites_migration_udacity_beamng_search".format(folder),
            UDACITY_SIM_NAME,
        ),
        ("{}/mapelites_migration_donkey_beamng_search".format(folder), DONKEY_SIM_NAME),
        (
            "{}/mapelites_migration_donkey_udacity_search".format(folder),
            DONKEY_SIM_NAME,
        ),
    ]
    if args.datetime_str_donkey is not None:
        migration_folders_env_names.extend(
            [
                (
                    "{}/mapelites_migration_beamng_donkey_search".format(folder),
                    BEAMNG_SIM_NAME,
                ),
                (
                    "{}/mapelites_migration_udacity_donkey_search".format(folder),
                    UDACITY_SIM_NAME,
                ),
            ]
        )
    ...
    for str_datetime, env_name, num_runs in str_datetime_env_name_num_runs_list:
        ...
        for quality_metric in QUALITY_METRICS_NAMES:
            ...
            logg.info(
                "Bounds {} for {}: ({}, {})".format(
                    ...
                    os.path.join(
                        folder,
                        "{}_{}_{}_all".format(
                            report_name_suffix, env_name, str_datetime
                        ),
                    ),
                    min_quality_metric,
                    max_quality_metric,
            ...
            quality_metric_dict[quality_metric] = (
                ...
                os.path.join(
                    folder,
                    "{}_{}_{}_all".format(report_name_suffix, env_name, str_datetime),
                ),
            )
    ...
    for (
        env_name,
        quality_metric_dicts,
    ) in quality_metric_dicts_across_simulators.items():
        if args.datetime_str_donkey is not None:
            # cross-validation
            assert (
                len(quality_metric_dicts) == 3
            ), "{} has more or less than 3 folders. Found: {}".format(
                env_name, len(quality_metric_dicts)
            )
        else:
            assert (
                len(quality_metric_dicts) == 2
            ), "{} has more or less than 2 folders. Found: {}".format(
                env_name, len(quality_metric_dicts)
            )
            ...
            for quality_metric in quality_metric_dict.keys():
                ...
                for key in quality_metric_map.keys():
                    if quality_metric_map[key] != FILL_VALUE:
                        quality_metric_map[key] = (quality_metric_map[key] - _min) / (
                            _max - _min
                        )
                        assert (
    ...
    for (
        ...
        for quality_metric_dict in quality_metric_dicts:
            for quality_metric in quality_metric_dict.keys():
                ...
                logg.info(
                    "Saving map for quality metric {} in {}".format(
                        quality_metric, folder_for_saving_map
                    )
                )
                plot_raw_map_of_elites(
