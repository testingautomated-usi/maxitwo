...
if __name__ == "__main__":
    ...
    for i, data in enumerate(dataset):
        if (
            opt.num_test != -1 and i >= opt.num_test
        ...
        print(
            "Inference time: {:.2f}s. {}/{}".format(
                time.perf_counter() - start_time, i, len(dataset)
            )
        )
        ...
        if len(img_path) > 0:
            image_dir = os.path.join(
                opt.dataroot[: opt.dataroot.rindex("/")],
                "translated{}".format(opt.model_suffix),
            )
            ...
            for label, im_data in visuals.items():
                ...
                if label == "fake":
                    ...
                    assert (
                        opt.original_image_size is not None
                        and len(opt.original_image_size) > 0
                    ), "Original image size not assigned"
                    image_size = image_size_after_crop(
                        env_name=sim_name, original_image_size=opt.original_image_size
                    )
                    pil_image = Image.fromarray(im)
