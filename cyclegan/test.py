...
if __name__ == '__main__':
        ...
        if len(img_path) > 0:
            ...
            for label, im_data in visuals.items():
                ...
                if label == "fake":
                    ...
                    assert opt.original_image_size is not None and len(opt.original_image_size) > 0, \
                        "Original image size not assigned"
                    image_size = image_size_after_crop(
                        env_name=sim_name,
                        original_image_size=opt.original_image_size
                    )
                    pil_image = Image.fromarray(im)
    ...
    if len(fake_obs) > 0:
        archive_name = opt.archive_filepath[opt.archive_filepath.rindex('/') + 1:opt.archive_filepath.rindex('.')] \
