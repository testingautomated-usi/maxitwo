...
from config import SIMULATOR_NAMES, BEAMNG_SIM_NAME, DONKEY_SIM_NAME, UDACITY_SIM_NAME, MOCK_SIM_NAME
...
def get_road(road_points: List[Point], control_points: List[Point], road_width: int, simulator_name: str) -> Road:
    assert simulator_name in SIMULATOR_NAMES, 'Simulator name {} not supported. Choose between: {}'.format(simulator_name, SIMULATOR_NAMES)
